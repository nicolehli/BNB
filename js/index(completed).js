        ...
        if (result.NavJoinCategoryResults) {
            for (var i = 0; i<result.NavJoinCategoryResults.length; i++) {
                ...
                try {
                    ...
                    console.log(pin.metadata.neighborhood + ": " + categoryId + ": " + pin.results[categoryId]);
                }
                catch (err) {
            ...
            console.log(pin.metadata.neighborhood + ": Service Returned Successfully, but with no data.");
        }
    }).fail(function (error) {
...
function CalculatePinScore(pin) {
    ...
    for (var i=0; i<category_results.length; i++) {
        score += (pin.results[category_results[i]]) * Math.pow(10,settings.categories[category_results[i]].value);
    }
...
/**
 * Updates the category values before calculating the pin scores.
 *
 * @private
 */
function UpdateCategoryValues() {
    ...
    for (var i=0; i<category_keys.length; i++) {
        settings.categories[category_keys[i]].value = parseInt(jQuery("#"+category_keys[i]).val());
    }
}
